...
class ExcelProcessor:
    def __init__(self):
        ...
        self.pattern_definitions = {
            ...
            "pattern7": {
                "headers": {3: "出库日期", 13: "客户名称", 6: "商品名称", 7: "品种规格", 9: "数量", 8: "批号"},
                "mapping": {3: 1, 6: 2, 7: 3, 8: 4, 13: 5, 9: 6}
            },
            "pattern8": {
            ...
            "pattern11": {
                "headers": {7: "出库日期", 20: "下游收货方名称", 22: "产品名称", 23: "产品规格", 25: "数量", 26: "原始批号"},
                "mapping": {7: 1, 22: 2, 23: 3, 26: 4, 20: 5, 25: 6}
            }
        }
        
        # 商业单位映射
        self.business_units = {
            "pattern1": "宜昌市瑞康医药有限责任公司",
            "pattern2": "九州通医药集团股份有限公司",
            "pattern10": "九州通医药集团股份有限公司",
            "pattern3": "湖北人福医药集团有限公司",
            "pattern4": "湖北华富康药业有限公司",
            "pattern5": "上药控股湖北有限公司",
            "pattern6": "南京医药湖北有限公司",
            "pattern7": "华润湖北医药有限公司",
            "pattern8": "国药控股湖北有限公司",
            "pattern9": "荆门市医药药材有限公司",
            "pattern11": "荆门市医药药材有限公司"
        }
    ...
    def create_summary_file(self) -> Optional[str]:
        ...
        try:
            ...
            # 设置表头
            headers = ["商业单位", "日期", "品种", "规格", "批号", "流向单位", "数量"]
            ...
            for start_row in range(2, last_row + 1, batch_size):
                ...
                try:
                    ...
                    for row_data in batch_data:
                        ...
                        # 添加商业单位信息到第一列
                        business_unit = self.business_units.get(pattern, "未知")
                        new_row.insert(0, business_unit)
        ...
        try:
            ...
            for i in range(0, len(result), batch_size):
                ...
                try:
                    ...
                    ws.range(f"A{current_row}:G{end_row}").value = batch
                except Exception as e:
                    print(f"写入批次数据失败: {e}")
                    # 如果批量写入失败，尝试逐行写入
                    for j, row in enumerate(batch):
                        try:
                            ws.range(f"A{current_row + j}:G{current_row + j}").value = [row]
                        except:
                            continue
    ...
    def cleanup_work_dir(self) -> None:
        """清空work_dir文件夹内的xlsx和xls文件"""
        try:
            print("正在清空工作目录中的Excel文件...")
            ...
            print(f"清理完成: 成功删除 {success_count} 个文件")
            
            if failed_files:
                print(f"删除失败的文件 ({len(failed_files)} 个):")
